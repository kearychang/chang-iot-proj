...
void sllib::setOnSingle(){
    float it = 0;
    runningFunction = 0;
    it = 255*(float)brightness/100;
    analogWrite(_pin, (int)it);
}
...
void sllib::setBrightness(int val){
    if (val >= 0 && val <= 100) {
        brightness = val;
    }
}

int sllib::getBrightness(){
    return brightness;
}
...
void sllib::breathSingle(int speed){
    if((milOld + 1) < millis() ){
        milOld = millis();
        float it = 0;
        it = (exp(sin(millis()/(float)speed*PI)) - 0.36787944)*108.0*(float)brightness/100;
        analogWrite(_pin, (int)it);
    }
}
...
void sllib::flickerSingle(int intMin, int intMax){
    if((milOld + 100) < millis()){
        milOld = millis();
        float it = 0;
        it = random(intMin,intMax)*(float)brightness/100;
        analogWrite(_pin, (int)it);
    }
}
...
void sllib::flickerSingle(){
    if((milOld + 100) < millis()){
        milOld = millis();
        float it = 0;
        it = random(80,255)*(float)brightness/100;
        analogWrite(_pin, (int)it);
    }
}
